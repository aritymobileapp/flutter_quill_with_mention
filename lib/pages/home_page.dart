...
class _HomePageState extends State<HomePage> {
  ...
  ValueNotifier<List<HashTagSearchResponseBean>> hashTagWordList = ValueNotifier([]);
  ValueNotifier<List<AtMentionSearchResponseBean>> atMentionSearchList = ValueNotifier([]);
  ...
  final _tempAtMentionList = [
    AtMentionSearchResponseBean(firstName: 'Tom', userName: 'tom123', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Tod', userName: 'tony', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Toe', userName: 'tory', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Jeck', userName: 'jack', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'John', userName: 'John', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Mical', userName: 'mical_mishra', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Obama', userName: 'obama_fans', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Putin', userName: 'putin', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Modi', userName: 'modi', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Targen', userName: 'targen', id: '80935823948569'),
    AtMentionSearchResponseBean(firstName: 'Tommy', userName: 'tomi_', id: '80935823948569'),
  ];
  ...
  void editorListener() {
    try {
      ...
      if (value is String && value.trim().isNotEmpty) {
        ...
        if (newString == '#') {
          _taggingCharector = '#';
          if (_hashTagOverlayEntry == null && !(_hashTagOverlayEntry?.mounted ?? false)) {
        ...
        if ((newString == ' ' || newString == '\n') && _hashTagOverlayEntry != null && _hashTagOverlayEntry!.mounted) {
          _removeOverLay();
        ...
        if (lastHashTagIndex != -1 && _hashTagOverlayEntry != null && (_hashTagOverlayEntry?.mounted ?? false)) {
          var newWord = value.substring(lastHashTagIndex!, value.length).replaceAll('\n', '');
                ...
                child: Container(
                  ...
                  child: _taggingCharector == '#'
                      ? ValueListenableBuilder(
                          ...
                          builder: (BuildContext context, List<HashTagSearchResponseBean> value, Widget? child) {
                            return ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                return GestureDetector(
                                  onTap: () {
                                    _onTapOverLaySuggestionItem(value[index].name!);
                                  },
                                  child: ListTile(
                      ...
                      : ValueListenableBuilder(
                          ...
                          builder: (BuildContext context, List<AtMentionSearchResponseBean> value, Widget? child) {
                            return ListView.builder(
                              padding: EdgeInsets.zero,
                              itemCount: value.length,
                              shrinkWrap: true,
                              itemBuilder: (BuildContext context, int index) {
                                ...
                                return GestureDetector(
                                  onTap: () {
                                    _onTapOverLaySuggestionItem(data.userName!, userId: data.userName);
                                  },
                                  child: ListTile(
                                    leading: CachedNetworkImage(
                                      ...
                                      imageBuilder: (context, imageProvider) => Container(
                                        height: 30,
                                        width: 30,
                                        decoration: BoxDecoration(image: DecorationImage(image: imageProvider, fit: BoxFit.cover), shape: BoxShape.circle),
                                      ),
  ...
  void _onTapOverLaySuggestionItem(String value, {String? userId}) {
    ...
    _controller!.replaceText(_lastHashTagIndex!, _controller!.selection.extentOffset - _lastHashTagIndex, value, null);
    _controller!.updateSelection(
        TextSelection(
            baseOffset: _lastHashTagIndex - 1, extentOffset: _controller!.selection.extentOffset + (value.length - (_controller!.selection.extentOffset - _lastHashTagIndex))),
        ChangeSource.LOCAL);
    if (_taggingCharector == '#') {
      /// You can add your own web site
      _controller!.formatSelection(LinkAttribute('https://www.google.com/search?q=$value'));
    } else {
      /// You can add your own web site
      // _controller!.formatSelection(LinkAttribute('mentionUser(this)'));
      _controller!.formatSelection(LinkAttribute('mentionUser(this)'));
    }
  ...
  Future<void> _getAtMentionSearchList(String? query) async {
    ...
    final tempList = _tempAtMentionList.where((e) => e.userName!.toLowerCase().contains(query.toString().split(" ").first.toLowerCase())).toList();

    // print("tempList----${tempList[0].userName.toString()}");

    /// you can call api here to get the list
    try {
      atMentionSearchList.value = tempList;
    } catch (e) {
    ...
    return SafeArea(
      child: Column(
        ...
        children: <Widget>[
          ...
          TextButton(
              onPressed: () {
                var deltaJson = _controller?.document.toDelta().toJson();
                print("GEt TExt : ${DeltaToHTML.encodeJson(deltaJson!).toString()}");
                print(
                    "GEt TExt : ${DeltaToHTML.encodeJson(deltaJson!).toString().replaceAll("href='mentionUser(this)'", "href=\"javascript:void(0)\" onclick=\"mentionUser(this)\"")}");
                // var json = jsonEncode(_controller.document.toDelta().toJson());
                // log("${json}");
              },
