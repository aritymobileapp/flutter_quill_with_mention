...
class RawEditorState extends EditorState
  ...
  Widget build(BuildContext context) {
    ...
    final constraints = widget.expands ? const BoxConstraints.expand() : BoxConstraints(minHeight: widget.minHeight ?? 0.0, maxHeight: widget.maxHeight ?? double.infinity);
  ...
  void _handleCheckboxTap(int offset, bool value) {
    if (!widget.readOnly) {
      ...
      widget.controller.toolbarButtonToggler = {Attribute.list.key: attribute, Attribute.header.key: Attribute.header};
  ...
  EditableTextLine _getEditableTextLineFromNode(Line node, BuildContext context) {
    ...
    final editableTextLine = EditableTextLine(node, null, textLine, 0, _getVerticalSpacingForLine(node, _styles), _textDirection, widget.controller.selection,
        widget.selectionColor, widget.enableInteractiveSelection, _hasFocus, MediaQuery.of(context).devicePixelRatio, _cursorCont);
  ...
  Future<void> pasteText(SelectionChangedCause cause) async {
    ...
    if (widget.controller.copiedImageUrl != null) {
      ...
      widget.controller.replaceText(index, length, BlockEmbed.image(copied.item1), null);
      if (copied.item2.isNotEmpty) {
        widget.controller.formatText(getImageNode(widget.controller, index + 1).item1, 1, StyleAttribute(copied.item2));
      }
      widget.controller.copiedImageUrl = null;
...
class _UpdateTextSelectionToAdjacentLineAction<T extends DirectionalCaretMovementIntent> extends ContextAction<T> {
  ...
  void invoke(T intent, [BuildContext? context]) {
    ...
    final newExtent = shouldMove ? currentRun.current : (intent.forward ? TextPosition(offset: state.textEditingValue.text.length) : const TextPosition(offset: 0));
